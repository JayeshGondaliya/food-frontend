...
const MyOrdersPage = () => {
  ...
  const handleStatusUpdate = useCallback((data: { orderId: string; status: string }) => {
    setOrders((prev) =>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <div key={order._id} className="rounded-lg border border-border bg-card p-5 animate-fade-in">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                <div>
                  <p className="text-xs text-muted-foreground">Order #{order._id.slice(-6)}</p>
              ...
              <div className="space-y-1 text-sm">
                {order.items.map((item, i) => (
                  <div key={i} className="flex justify-between text-foreground">
                    <span>{typeof item.menuItem === "object" ? item.menuItem.name : "Item"} × {item.quantity}</span>
                    <span>{typeof item.menuItem === "object" ? `$${(item.menuItem.price * item.quantity).toFixed(2)}` : ""}</span>
                  </div>
                ))}
              </div>
              ...
              {order.deliveryDetails && (
                <div className="mt-2 text-xs text-muted-foreground">
                  📍 {order.deliveryDetails.address} • 📞 {order.deliveryDetails.phone}
                </div>
              )}
            </div>
          ))}
